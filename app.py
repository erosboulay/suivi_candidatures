from sqlalchemy import create_engine, text, MetaData, select, func, distinct, insert, delete,  update
...
@app.route('/edit_candidature', methods=['POST'])
def edit_candidature():
    entreprise = request.form.get('Entreprise')
    lien = request.form.get('Lien candidature')
    source = request.form.get('Source')
    poste = empty_to_null(request.form.get('Poste'))
    ...
    c_id = int(request.form.get("ID"))

    with Session(engine) as session:
        existing = session.execute(select(Candidature).where(Candidature.c.id == c_id)).fetchone()
        if not existing:
            print("id not found")

        stmt = update(Candidature).where(Candidature.c.id == c_id).values(
                            entreprise = entreprise, 
                            ville = ville, 
                            pays = pays,  
                            poste = poste, 
